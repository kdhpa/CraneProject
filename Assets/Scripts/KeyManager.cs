...
public class KeyManager : MonoBehaviour
{
    ...
    private Dictionary<InputAction, Action<InputAction.CallbackContext>> buttonCallbacks = new Dictionary<InputAction, Action<InputAction.CallbackContext>>();
    ...
    private void InputSetting()
    {
        ...
        if (buttonActions != null && buttonActions.Count > 0)
        {
            for( int i = 0; i < buttonActions.Count; i++ )
            {
                ...
                {
                    // 기존 콜백이 있으면 제거
                    if (buttonCallbacks.ContainsKey(action))
                    {
                        action.performed -= buttonCallbacks[action];
                    }

                    // 새로운 콜백 생성 및 등록
                    Action<InputAction.CallbackContext> callback = (InputAction.CallbackContext context) =>
                    {
